...
def main(argv=None):
    ...
    ap.add_argument("--drop-weights", action="store_true",
                    help="delete the checkpoint once its metrics are written; "
                         "the numbers live in the json, the weights do not "
                         "need to survive it")
    ...
    loader = build_dataloader(args.data, args.split,
                              classes=dataset_classes(args.data),
                              imgsz=args.imgsz,
    ...
    if args.json:
        ...
        if args.drop_weights:
            os.remove(args.weights)
            print(f"[cleanup] 已删除 {args.weights}")
    return res
